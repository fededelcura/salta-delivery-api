...
const file = path.resolve(__dirname, '..', '..', 'database', '009_reportes_guardados.sql');
...
const check = await pool.request().query(`
  SELECT COUNT(*) AS ok FROM sys.tables WHERE name = 'reportes_guardados'
`);
console.log(check.recordset[0]);
await pool.close();
console.log('Migración 009 OK');
