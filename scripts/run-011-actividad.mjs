...
const file = path.resolve(__dirname, '..', '..', 'database', '011_cadete_actividad.sql');
...
const check = await pool.request().query(`
  SELECT
    (SELECT COUNT(*) FROM sys.tables WHERE name='cadete_sesiones') AS tabla,
    (SELECT COUNT(*) FROM dbo.cadete_sesiones) AS sesiones
`);
console.log(check.recordset[0]);
await pool.close();
console.log('Migración 011 OK');
