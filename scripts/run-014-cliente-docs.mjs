...
const file = path.resolve(__dirname, '..', '..', 'database', '014_cliente_direccion_dni.sql');
...
const check = await pool.request().query(`
  SELECT COL_LENGTH('dbo.clientes','direccion') AS direccion,
         COL_LENGTH('dbo.clientes','fotos_documentos') AS fotos
`);
console.log(check.recordset[0]);
await pool.close();
console.log('Migración 014 OK');
