...
const file = path.resolve(__dirname, '..', '..', 'database', '013_cadete_direccion_docs.sql');
...
const check = await pool.request().query(`SELECT COL_LENGTH('dbo.cadetes','direccion') AS direccion`);
console.log(check.recordset[0]);
await pool.close();
console.log('Migración 013 OK');
